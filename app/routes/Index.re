  ...
  Js.Promise.
    (
      ...
      |> then_(json => {
           ...
           Js.log2("Loader", json);
           resolve(Bindings.RemixNode.json(json));
         })
      |> catch(err => {
           ...
           resolve(Bindings.RemixNode.json(body));
         })
    );
    // |> ignore
};
...
let default = () => {
  ...
  Js.log2("useLoader", json);
  // let data = json |> Json.parseOrRaise |> Json.Decode.(decodeCharactersResultsObject)
  // Json.parseOrRaise json |> Decode.(array decodeCharactersResultsObject);
